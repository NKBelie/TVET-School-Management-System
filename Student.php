...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validation
    $errors = [];
    if ($id <= 0) $errors[] = "Valid Student ID required.";
    if (empty($FName)) $errors[] = "Full Name required.";
    if (!in_array($Gender, ['Male', 'Female'])) $errors[] = "Valid Gender required.";
    if (empty($Email) || !filter_var($Email, FILTER_VALIDATE_EMAIL)) $errors[] = "Invalid Email Format.";
    if (empty($Phone) || !preg_match('/^\+?[1-9]\d{1,14}$/', $Phone)) $errors[] = "Valid Phone required (e.g., +1234567890).";
    if (empty($date) || strtotime($date) > time()) $errors[] = "Valid Enrollment Date required (not in future).";
    ...
    if (empty($errors)) {
        ...
        if (!$checkStmt) {
            ...
            if ($checkStmt->get_result()->num_rows > 0) {
                $message = "<span style='color:red;'>Error: Student Already Exists (ID or Email taken).</span>";
            } else {
                // Secure INSERT
                $stmt = $conn->prepare("INSERT INTO student (Student_Id, Full_Name, Gender, Email, Phone, Enrollment_Date, Status) VALUES (?, ?, ?, ?, ?, ?, ?)");
                if (!$stmt) {
                    $message = "<span style='color:red;'>INSERT prepare failed: " . htmlspecialchars($conn->error) . ". Add missing columns (e.g., Gender) to 'student' table.</span>";
                } else {
                    $stmt->bind_param("sssssss", $id, $FName, $Gender, $Email, $Phone, $date, $status);
                    if ($stmt->execute()) {
                        $message = "<span style='color:green;'>Student Registered Successfully!</span>";
                        $_POST = []; // Clear form
                    } else {
    ...
    <div class="content">
        <center><br><br><br>
            ...
            <form method="POST" action="Student.php">
                ...
                <input type="text" name="phone" value="<?php echo htmlspecialchars($_POST['phone'] ?? ''); ?>" placeholder="+1234567890" required><br><br>

                <label>Enrollment Date:</label><br>
                <input type="date" name="date" value="<?php echo htmlspecialchars($_POST['date'] ?? date('Y-m-d')); ?>" required><br><br>
