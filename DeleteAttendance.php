...
if ($id <= 0) {
    ...
    $stmt = $conn->prepare("SELECT Attendance_Id, Student_Id, Course_Id, Adate, Status FROM `attendance` WHERE Attendance_Id = ?");
    ...
    if (!$attendance) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm_delete']) && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'] ?? '')) {
            ...
            if ($deleteStmt->execute()) {
                ...
                $success = true;
                header("Location: VAttendance.php?success=deleted");
                exit;
            } else {
...
<div class="content">
    <center><br><br>
        ...
        <?php else: ?>
            <p>Are you sure you want to delete attendance for Student ID: <?php echo htmlspecialchars($attendance['Student_Id']); ?> in Course ID: <?php echo htmlspecialchars($attendance['Course_Id']); ?> on <?php echo htmlspecialchars($attendance['Adate']); ?> (Status: <?php echo htmlspecialchars($attendance['Status']); ?>)?</p>
            <form method="POST">
