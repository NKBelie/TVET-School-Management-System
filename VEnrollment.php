...
if (!isset($conn) || !$conn || $conn->connect_error) {
    ...
    $result = $conn->query("
        SELECT enrollment.Enroll_Id, enrollment.Student_Id, enrollment.Course_Id, enrollment.Enrollment_Date, enrollment.Status, 
    ...
    ");
    if ($result === false) {
        $message = "Failed to fetch enrollment records: " . htmlspecialchars($conn->error);
        error_log("Enrollment Query Error: " . $conn->error);
    }
}
?>
<!DOCTYPE html>
        ...
        <?php elseif ($result && $result->num_rows > 0): ?>
            <table>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($row['Enroll_Id']); ?></td>
                        <td><?php echo htmlspecialchars($row['Full_Name']); ?></td>
                        <td><?php echo htmlspecialchars($row['Title']); ?></td>
                        <td><?php echo htmlspecialchars($row['Enrollment_Date']); ?></td>
                        <td><?php echo htmlspecialchars($row['Status']); ?></td>
                        <td>
                            <a href="EditEnrollment.php?id=<?php echo urlencode($row['Enroll_Id']); ?>"><button>Edit</button></a>
                            <a href="DeleteEnrollment.php?id=<?php echo urlencode($row['Enroll_Id']); ?>" onclick="return confirm('Are you sure?');"><button>Delete</button></a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </table>
            <?php $result->free(); ?>
        <?php else: ?>
            <p style="color:red;">No enrollment records found.</p>
        <?php endif; ?>
    </center>
</div>
