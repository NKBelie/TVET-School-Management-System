...
if (empty($id)) {
    ...
    $stmt = $conn->prepare("SELECT Instructor_Id, Full_Name, Specialization FROM `instructor` WHERE Instructor_Id = ?");
    ...
    if (!$instructor) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm_delete']) && hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'] ?? '')) {
            // DELETE (cascade to courses/enrollments if FK set)
            $deleteStmt = $conn->prepare("DELETE FROM `instructor` WHERE Instructor_Id = ?");
            $deleteStmt->bind_param("s", $id);
            if ($deleteStmt->execute()) {
                $message = "Instructor deleted successfully (courses cascaded).";
                $success = true;
                header("Location: VInstructor.php?success=deleted");
                exit;
            } else {
...
<div class="content">
    <center><br><br>
        ...
        <?php if ($message): ?>
            ...
            <a href="VInstructor.php"><button>Back to List</button></a>
        <?php else: ?>
            <p>Are you sure you want to delete instructor "<?php echo htmlspecialchars($instructor['Full_Name']); ?>" (Specialization: <?php echo htmlspecialchars($instructor['Specialization']); ?>)? This will cascade delete courses and related enrollments/grades.</p>
            <form method="POST">
